...
const joinInstitute = AsyncHandler(async (req, res) => {
    try {
       ...
       const result = await instituteAdminSchema.findByIdAndUpdate(_id,{$set:{request:[{type:"student",name:user.name,aadharCardNumber:user.aadharCardNumber,email:user.email}],}})
